...
@router.post("/csv")
async def upload_csv(file: UploadFile = File(...), user_name: str = "Me", user_title: str = ""):
    if not file.filename.endswith(".csv"):
        raise HTTPException(status_code=400, detail="Only CSV files accepted")

    contents = await file.read()
    df = parse_csv(contents)
    stats = build_graph(df, {"name": user_name, "title": user_title})

    return {
        "success": True,
        "user_id": make_id(user_name),
        "stats": stats,
